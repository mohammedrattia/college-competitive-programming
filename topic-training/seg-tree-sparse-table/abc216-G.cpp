...
void solve()
{
    ...
    rep(i, 0, m)
    {
        ll ones = arr[i][2] - st.calc(arr[i][0] - 1, arr[i][1]).value;
        if (ones <= 0)
            continue;
        ll l = arr[i][0] - 1, r = arr[i][1];
        while (l < r)
        {
            ll m = l + (r - l) / 2;
            if (((arr[i][1] - m) - st.calc(m, arr[i][1]).value) < ones)
                r = m - 1;
            else if (((arr[i][1] - m) - st.calc(m, arr[i][1]).value) > ones)
                l = m + 1;
            else
            {
                l = m;
