...
bool is_part(string &a, Hash &ha, string &b, Hash &hb)
{
    if (a.size() > b.size())
    {
        swap(a, b);
    ...
    pii hs = ha.getHashVal();
    rep(i, 0, (int)(b.size() - a.size()))
    {
        pii sub = hb.getRangeHashVal(i, i + a.size() - 1);
        if (sub == hs)
        {
            a = "";
            return true;
...
int inst(string &s1, Hash &h1, string &s2, Hash &h2, string &s3, Hash &h3)
{
    int len1 = 0, len2 = 0;
    ll n1 = s1.size(), n2 = s2.size(), n3 = s3.size();
    rrep(i, min(n1, n2) - 1, 0)
    {
        pii suf = h1.getRangeHashVal(n1 - i - 1, n1 - 1);
        pii pre = h2.getRangeHashVal(0, i);
        if (suf == pre)
        {
            len1 = i + 1;
            break;
    ...
    rrep(i, min(n2, n3) - 1, 0)
    {
        pii suf = h2.getRangeHashVal(n2 - i - 1, n2 - 1);
        pii pre = h3.getRangeHashVal(0, i);
        if (suf == pre)
        {
            len2 = i + 1;
            break;
...
void solve()
{
    ...
    pre();
    Hash h1 = Hash(s1), h2 = Hash(s2), h3 = Hash(s3);
    is_part(s1, h1, s2, h2);
    is_part(s2, h2, s3, h3);
    is_part(s1, h1, s3, h3);
    int ans = 1e9 + 7;
    ans = min(ans, inst(s1, h1, s2, h2, s3, h3));
    ans = min(ans, inst(s1, h1, s3, h3, s2, h2));
    ans = min(ans, inst(s2, h2, s1, h1, s3, h3));
    ans = min(ans, inst(s2, h2, s3, h3, s1, h1));
    ans = min(ans, inst(s3, h3, s1, h1, s2, h2));
    ans = min(ans, inst(s3, h3, s2, h2, s1, h1));
    cout << ans << endl;
}
