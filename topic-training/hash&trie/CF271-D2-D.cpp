...
void solve()
{
    ...
    vi is_bad(26);
    rep(i, 0, 26) is_bad[i] = 1 - (stat[i] - '0');

    pre();
    Hash hashed = Hash(str);

    unordered_set<ll> s;
    int l = 0, r = 0, cnt = 0, old_r = 0;
    while (r != n)
    {
        if (cnt + is_bad[str[r] - 'a'] > k)
        {
            rep(i, l, r)
            {
                rep(j, max(i, old_r), r) { s.insert(pair_hash(hashed.getRangeHashVal(i, j))); }
            }
            old_r = r - 1;
            while (cnt + is_bad[str[r] - 'a'] > k)
            {
                cnt -= is_bad[str[l++] - 'a'];
            }
        }
