...
// SEGMENT TREE
typedef ll segtype;
const segtype NEUTRAL_CASE = INF;
class TreeNode
{
public:
    TreeNode *left;
...
public:
    ...
    segtype get(int l, int r, int &cnt)
    {
        return getRec(l, r, cnt, tree.head, 0, sz);
    }
...
private:
    ...
    TreeNode *merge(TreeNode *left, TreeNode *right)
    {
        ...
        root->cnt = calcCnt(left->value, right->value, left->cnt, right->cnt);
        return root;
    }

    segtype getRec(int l, int r, int &cnt, TreeNode *x, int lx, int rx)
    {
        if (l >= rx || lx >= r || x == NULL)
            return NEUTRAL_CASE;
        if (lx >= l && rx <= r)
        ...
        int m = (lx + rx) / 2;
        int cnt1 = 0, cnt2 = 0;
        segtype s1 = getRec(l, r, cnt1, x->left, lx, m);
        segtype s2 = getRec(l, r, cnt2, x->right, m, rx);
        cnt = calcCnt(s1, s2, cnt1, cnt2);
        return merge(s1, s2);
    }

    int calcCnt(segtype a, segtype b, int cntA, int cntB)
    {
        if (a < b)
            return cntA;
        else if (a > b)
    ...
    void setRec(int i, segtype v, TreeNode *x, int lx, int rx)
    {
        ...
        x->cnt = calcCnt(x->left->value, x->right->value, x->left->cnt, x->right->cnt);
    }
