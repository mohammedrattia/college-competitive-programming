...
{
public:
    const segtype NEUTRAL_CASE = {0, vll(40ll, 0)};
    Tree tree;
    ll sz;
...
void solve()
{
    ...
    SegTree st = SegTree(arr, [](segtype a, segtype b) -> segtype
                         { 
                            segtype ret = {a.inv + b.inv, vll(40ll, 0)};
                            rep(i, 0, 40)
                            {
                                ret.freq[i] += a.freq[i] + b.freq[i];
                                rep(j, 0, i)
                                {
                                    ret.inv += a.freq[i] * b.freq[j];
                                }
                            }
                            return ret; });
    rep(i, 0, q)
