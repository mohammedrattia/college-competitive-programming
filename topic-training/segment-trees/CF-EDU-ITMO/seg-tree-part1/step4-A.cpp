...
void solve()
{
    ...
    SegTree stOdd = SegTree(odd, [](segtype a, segtype b) -> segtype
                            { return {a.value + b.value}; });
    SegTree stEven = SegTree(even, [](segtype a, segtype b) -> segtype
                             { return {a.value + b.value}; });
    ...
    rep(i, 0, m)
    {
        ...
        if (c)
        {
            ...
            l--;
            if (l % 2 == 0)
                cout << stEven.get(l, r).value - stOdd.get(l, r).value << endl;
            else
                cout << stOdd.get(l, r).value - stEven.get(l, r).value << endl;
        }
        else
        {
            ll idx, j;
