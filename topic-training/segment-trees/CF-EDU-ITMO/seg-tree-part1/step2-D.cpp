...
public:
    ...
    ll get(int x, int l)
    {
        return getRec(x, l, tree.head, 0, sz);
    }
...
private:
    ...
    ll getRec(int v, int l, TreeNode *x, int lx, int rx)
    {
        if (x == NULL || x->item.value < v || rx <= l)
        {
            return NEUTRAL_CASE;
        }
        if (rx - lx == 1)
            return lx;
        int m = (lx + rx) / 2;
        ll ret = getRec(v, l, x->left, lx, m);
        if (ret != -1)
            return ret;
        return getRec(v, l, x->right, m, rx);
    }
