...
template <typename T> T rng(T l, T r) { return uniform_int_distribution<T>(l, r)(rnd); }
...
ll modInverseFer(ll a, ll m) { return modPower(a, m - 2, m); }
vll fact;
void preFact(int n)
{
    fact = vll(n);
...
struct SegTree
{
    ...
    // Operation function (e.g., sum, min, max, gcd)
    segtype merge(segtype a, segtype b) { return {a.value + b.value}; }
    segtype single(ll v) { return {v}; }
    ...
    // Build from existing array
    void build(vector<ll> &a) { roots.push_back(build(a, 0, size)); }
    ...
    // Point Update
    void set(int i, ll v, int x) { roots.push_back(set(i, v, x, 0, size)); }
    ...
    // Range Query
    segtype calc(int l, int r, int x) { return calc(l, r, x, 0, size); }
...
struct SpaTable
{
    ...
    spatype merge(spatype a, spatype b) { return a + b; }
...
template <typename T> struct MergeSortTree
{
    ...
    T merge(const T &a, const T &b)
    {
        ...
        std::merge(a.vec.begin(), a.vec.end(), b.vec.begin(), b.vec.end(),
    ...
    void build(const vector<ll> &a) { build(a, 0, 0, size); }
    ...
    int get(int l, int r, ll target_x) { return get(l, r, target_x, 0, 0, size); }
